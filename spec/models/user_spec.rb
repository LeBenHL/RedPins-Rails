...
describe User do
  ...
  describe 'Get Like' do
     before(:each) do
       ...
       @user.likeEvent(@event5.id, true)
       @user.likeEvent(@event6.id, true)
       @user.likeEvent(@event7.id, true)
       @user.likeEvent(@event8.id, true)
       @user.likeEvent(@event9.id, true)
       @user.likeEvent(@event10.id, true)
     end
     
     it 'getLikes should return the first page of 5 likes' do
         response = @user.getLikes(1, 5)
         response[:errCode].should eq(RedPins::Application::SUCCESS)
         likes = Like.where(:user_id => @user.id).order("created_at DESC").limit(5).offset(0)
         likes.each do |like|
           event = like.event
           attributes = event.getAttributes(@user.id)
           response[:likedEvents].should include(attributes)
         end
         response[:likedEvents].length.should eq(5)
         response[:next_likes_page].should eq(2)
       end
